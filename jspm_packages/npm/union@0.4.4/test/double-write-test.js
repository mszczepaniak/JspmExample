...
var doubleWrite = false,
    server;
server = union.createServer({before: [function(req, res) {
...
vows.describe('union/double-write').addBatch({"When using union": {"an http server which attempts to write to the response twice": {
      topic: function() {
    ...
    }}}).addBatch({"When the tests are over": {"the server should close": function() {
      server.close();
    }}}).export(module);
